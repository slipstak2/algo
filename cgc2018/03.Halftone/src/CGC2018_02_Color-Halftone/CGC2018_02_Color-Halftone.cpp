...
int main(int argc, char* argv[]) {
   ...
   const char* imageFileName = "data/lena.bmp";

   RGBpixelMap imageOriginal;
   imageOriginal.readBmpFile(imageFileName);
   imageOriginal.toGray();

   RGBpixelMap imageOrderedDither = imageOriginal.toOrderedDither();
   RGBpixelMap imageErrorDiffusion = imageOriginal.toErrorDiffusion();


   images.push_back({&imageOriginal, "Original"});
   images.push_back({&imageOrderedDither, "Ordered dither 8"});
   images.push_back({&imageErrorDiffusion, "Error diffusion: Floyd-Steinberg"});
